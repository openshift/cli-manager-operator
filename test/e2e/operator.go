...
func teardownOperator() {
	...
	err := cliManagerClient.ClimanagersV1().CliManagers(operatorclient.OperatorNamespace).Delete(ctx, operatorclient.OperatorConfigName, metav1.DeleteOptions{})
	if err != nil && !apierrors.IsNotFound(err) {
		klog.Warningf("failed to delete CliManager CR: %v", err)
	}
